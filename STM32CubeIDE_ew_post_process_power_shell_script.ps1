...
# Check project id
If (-Not (Select-String -Path $Path -Pattern "com.st.stm32cube.ide.mcu.gnu.managedbuild.config.exe.debug.1659078198" -SimpleMatch -Quiet))
{
...
# Delete rte include
$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.st.stm32cube.ide.mcu.gnu.managedbuild.config.exe.debug.1659078198"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "MCU ARM GCC"}).tool|where {$_.name -eq "MCU GCC Linker"}).option|where {$_.name -eq "Libraries (-l)"}).listOptionValue|where {$_.value -eq "ewrte-m4-gcc"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }
...
$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.st.stm32cube.ide.mcu.gnu.managedbuild.config.exe.debug.1659078198"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "MCU ARM GCC"}).tool|where {$_.name -eq "MCU GCC Linker"}).option|where {$_.name -eq "Libraries (-l)"}).listOptionValue|where {$_.value -eq "ewgfx-m4-gcc"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.st.stm32cube.ide.mcu.gnu.managedbuild.config.exe.debug.1659078198"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "MCU ARM GCC"}).tool|where {$_.name -eq "MCU GCC Linker"}).option|where {$_.name -eq "Libraries (-l)"}).listOptionValue|where {$_.value -eq "ewgfx-m4-r90-gcc"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.st.stm32cube.ide.mcu.gnu.managedbuild.config.exe.debug.1659078198"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "MCU ARM GCC"}).tool|where {$_.name -eq "MCU GCC Linker"}).option|where {$_.name -eq "Libraries (-l)"}).listOptionValue|where {$_.value -eq "ewgfx-m4-r180-gcc"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }

$DeleteNode = (((((((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.st.stm32cube.ide.mcu.gnu.managedbuild.config.exe.debug.1659078198"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).folderInfo.toolChain)|where {$_.name -eq "MCU ARM GCC"}).tool|where {$_.name -eq "MCU GCC Linker"}).option|where {$_.name -eq "Libraries (-l)"}).listOptionValue|where {$_.value -eq "ewgfx-m4-r270-gcc"})
if($DeleteNode) { $target.RemoveChild($DeleteNode) | Out-Null }
...
######################### EXCLUDES FROM BUILD ############################
$target = (((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.st.stm32cube.ide.mcu.gnu.managedbuild.config.exe.debug.1659078198"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).sourceEntries.entry).excluding
...
if($CurrentColorFormat -ne $NewColorFormat)
{
    ...
    (((($xml.cproject.storageModule.cconfiguration|where {$_.id -eq "com.st.stm32cube.ide.mcu.gnu.managedbuild.config.exe.debug.1659078198"}).storageModule|where {$_.moduleId -eq "cdtBuildSystem"}).configuration|where {$_.name -eq "BUILD"}).sourceEntries.entry).excluding = $Newtarget
}
